...
class PageHome extends React.Component {
  ...
  componentDidMount() {
    ...
    projects.map((project) => {
      var img = new Image();
      img.src = `${this.state.github_url}/${RES_DIR}/${project.img}`;
      return true;
    });
  }
  ...
  render() {
    ...
    if (this.state.windowWidth < 800) {
      ...
      previewPane = (
        <PreviewPane github_url={this.state.github_url} projects={projects} project={this.state.projectHovered} />
      );
    ...
    return (
      <div id="PageHome">
        ...
        <div id="preview-container">{previewPane}</div>
      </div>
    );
  }
}
